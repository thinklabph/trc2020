/*  Toggle Switch with Bool Variable
...
bool switchState = false;                   // bool variable can store true(5V or HIGH) or false (0V or LOW) only
...
void loop() {
  if (digitalRead(switchPin) == LOW) {      // switch is pressed - pullup keeps pin high normally
    delay(500);                             // delay to wait for switch to be unpressed (debounce) - avoids unnecessary mechanical presses
