/* LEARN HOW TO CODE ARDUINO by ThinkLab 2020
...
void countNumber (int num) {
  ...
  int i = 0;                          // while is preferred when the number of iterations are not yet defined
  while (i <= num) {
    Serial.print(i);
    Serial.print(" ");
    delay(500);
  ...
  do {                                // do.. while will always run once since the condition is tested at the end of the loop
    Serial.println(i);
    delay(500);
    i += 2;
  } while (i <= num);
...
void compare (int num1, int num2) {
  ...
  else if ((num1 > num2) && ((num1 + num2) <= 1000)) {      // both conditions must be true before doing this command
    Serial.print(num1);
